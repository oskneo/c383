...
func TestPointScaling2(t *testing.T) {
	...
	k := Point{52, 20}
	k.Scale(0.5)
	assert.Equal(t, k.x, p.x)
	assert.Equal(t, k.y, p.y)
}
...
func TestPointRotate2(t *testing.T) {
	...
	p.Rotate(math.Pi / 4)
	k := Point{1, 0}
	k.Rotate(-math.Pi / 4 * 7)
	assert.Equal(t, float32(k.x), float32(p.x))
...
func TestMeanStddev(t *testing.T) {
	...
	mean1, stddev1 := MeanStddev(arr1, 2)
	mean2, stddev2 := MeanStddev(arr2, 3)
	assert.Equal(t, float32(mean1), float32(mean2/2))
	assert.True(t, float32(stddev1) < float32(stddev2))
